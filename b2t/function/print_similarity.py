...
def print_similarity(keyword_0, keyword_1, dist_0, dist_1, df_0):
    ...
    for keyword, diff in zip(keyword_0, dist_0):
        ...
        result["Images"].append(", ".join(biased_dataset['image'].values))
        result["ImagesCount"].append(biased)
        if biased == 0:
            result["Acc."].append("")
            result["Bias"].append("")
            continue
    ...
    print(tabulate(diff.drop(columns = ["Images"]), headers='keys', showindex=False))
    return diff