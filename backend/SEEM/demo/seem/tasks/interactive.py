...
def interactive_infer_image(model, audio_model, image, tasks, device, refimg=None, reftxt=None, audio_pth=None, video_pth=None):
    ...
    if 'Example' in tasks:
        ...
        images = torch.from_numpy(refimg_ori.copy()).permute(2,0,1).to(device)
    ...
    if 'Panoptic' in tasks:
        ...
        # res = demo.get_image()
        return pano_seg.cpu(), pano_seg_info
    else:
...
def interactive_infer_video(model, audio_model, image, tasks, device, refimg=None, reftxt=None, audio_pth=None, video_pth=None):
    if 'Video' in tasks:
